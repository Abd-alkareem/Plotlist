...
//format the category part
    document.querySelectorAll(".category .controle button ").forEach((s, ind) => {
          s.addEventListener("click", () => {
            document.querySelectorAll(".category .controle button ").forEach((e) => {
                e.classList.remove("active");
                document.querySelectorAll(".category .holder .slide ").forEach((e) => {
                    e.classList.add("hidden");

                    setTimeout(() => {
                      e.classList.remove("active");
                      e.classList.remove("hidden");
                      document.querySelectorAll(".category .holder .slide ")[ind].classList.add("active");
                    }, 300);
                  });
              });
